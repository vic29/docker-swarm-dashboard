...
console.log('Starting Gerrit info collector module');
...
const gerritHost = process.env.GERRIT_URL || '';
const gerritUser = process.env.GERRIT_USER || '';
const gerritPwd = process.env.GERRIT_PWD || '';
const collectReviewInfo = process.env.GERRIT_COLLECT && gerritHost && gerritUser;
            ...
            function next() {
                if (index < stacks.length) {
                    ...
                    if ( found && found[0] ) {
                        ...
                        console.log('Get review info from gerrit: ', reviewNum);
                        ...
                        shell.exec(cmd, {async:true,silent:true}, function(code, stdout, stderr) {
                            if ( stdout.startsWith(")]}'") ) { stdout = stdout.substring(4); }
                            if ( stdout.startsWith("Not found") ) { stdout = "{}"; }
                            try {
...
function getUserNameFromGerrit(userId) {
    if ( userCache[userId] && userCache[userId].name ) { return userCache[userId].name; }
    let name = '';
    try {
        let stdout = shell.exec('curl --user ' + gerritUser + ':' + gerritPwd + ' http://' + gerritHost + ':80/a/accounts/' + userId + '/name', {async:false,silent:true}).stdout;
        if ( stdout.startsWith(")]}'") ) { stdout = stdout.substring(4); }
        if ( stdout.startsWith("Not found") ) { stdout = "{}"; }
        name = stdout.replaceAll('"', '').allTrim();

        if ( !userCache[userId] ) { userCache[userId] = { name:null, email:null }; }
...
function getUserEmailFromGerrit(userId) {
    if ( userCache[userId] && userCache[userId].email ) { return userCache[userId].email; }
    let userEmail = '';
    try {
        let stdout = shell.exec('curl --user build:s7UJ2xPl http://' + gerritHost + ':80/a/accounts/' + userId + '/emails', {async:false,silent:true}).stdout;
        if ( stdout.startsWith(")]}'") ) { stdout = stdout.substring(4); }
        if ( stdout.startsWith("Not found") ) { stdout = "{}"; }
